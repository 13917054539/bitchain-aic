...
public class MemberWalletService extends BaseService {
    ...
    public MessageResult recharge(Coin coin, String address, BigDecimal amount, String txid) {
        MemberWallet wallet = findByCoinAndAddress(coin, address,address);
        if (wallet == null) {
            return new MessageResult(500, "wallet cannot be null");
        }
        if (amount.compareTo(BigDecimal.ZERO) <= 0) {
     ...
     * @return
     */
    public MemberWallet findByCoinAndAddress(Coin coin, String address,String addressEth) {
        return memberWalletDao.findByCoinAndAddressOrAddressEth(coin, address,addressEth);
    }
    ...
    public List<MemberWallet> findAllByMemberIdOrderByVersionDesc(Long memberId) {
        return memberWalletDao.findAllByMemberIdOrderByVersionDesc(memberId);
    }

    public List<MemberWallet> findAllByMemberIdOrderByVersionDesc(Long memberId, Sort sort) {
        return memberWalletDao.findAllByMemberIdOrderByVersionDesc(memberId,sort);
    }
    /**
     * 冻结钱包
     *
       ...
       // AefPatternConf aefPatternConf = aefPatternConfDao.getAefPatternConf(1);
        if (ret == 1) {
            ...
            if (is > 0) {
                ...
                //增加额度
                if(order.getCoin().getName().equals("CTO")){
                    //计算奖励额度(3倍)
                    BigDecimal sub = FunctionUtils.mul(BigDecimalUtils.sub(order.getNumber(), order.getCommission()), new BigDecimal("1.5"), 8);
                    //获取CTO钱包信息
                    MemberWallet memberWalletCto = findByOtcCoinAndMemberId(order.getCoin(), memberWallet.getMemberId());
                    bIPBMathService.updateCtoquota(memberWalletCto.getId(), sub, 1, memberWalletCto.getCoin().getName(), TransactionType.COIN_RELEASE);
                }
            } else {
                throw new InformationExpiredException("Information Expired");
            }
            ...
            if (is > 0) {
                ...
                if(order.getCoin().getName().equals("CTO")){
                    //计算奖励额度(3倍)
                    BigDecimal sub = FunctionUtils.mul(order.getNumber(), new BigDecimal("3"), 8);
                    //获取CTO钱包信息
                    MemberWallet memberWalletCto = findByOtcCoinAndMemberId(order.getCoin(), memberWallet.getMemberId());
                    bIPBMathService.updateCtoquota(memberWalletCto.getId(), sub, 1, memberWalletCto.getCoin().getName(), TransactionType.COIN_RELEASE);
                }
    ...
    public void increaseBalanceAndToReleased(Long walletId,BigDecimal amount){
        memberWalletDao.increaseBalanceAndToReleased(walletId,amount);
    }
