    ...
    public void auditPass(MemberApplication application) {
       ...
       if(needRealName==1){
           ...
           *//**
            * 实名注册奖励用户5000
            * *//*
           //BigDecimal amountOwn = JSONObject.parseObject(rewardPromotionSetting.getInfo()).getBigDecimal("own");
           BigDecimal amountOwn =BigDecimal.ZERO;
           if(amountOwn.compareTo(BigDecimal.ZERO)>0){
               MemberWallet ownMemberWallet = memberWalletService.findByCoinAndMember(rewardPromotionSetting.getCoin(), member);

               if(ownMemberWallet.getToReleased()==null){
                   ownMemberWallet.setToReleased(BigDecimal.ZERO);
               }

               ownMemberWallet.setToReleased(BigDecimalUtils.add(ownMemberWallet.getToReleased(), amountOwn));
               //memberWallet1.setBalance(BigDecimalUtils.add(memberWallet1.getBalance(), amount1));
               memberWalletService.save(ownMemberWallet);
               //奖励记录
               RewardRecord rewardRecordOwn = new RewardRecord();
               rewardRecordOwn.setAmount(amountOwn);
               rewardRecordOwn.setCoin(rewardPromotionSetting.getCoin());
               rewardRecordOwn.setMember(member);
               rewardRecordOwn.setRemark(rewardPromotionSetting.getType().getCnName());
               rewardRecordOwn.setType(RewardRecordType.PROMOTION);
               rewardRecordService.save(rewardRecordOwn);
               //交易流水
               MemberTransaction memberTransactionOwn = new MemberTransaction();
               memberTransactionOwn.setFee(BigDecimal.ZERO);
               memberTransactionOwn.setAmount(amountOwn);
               memberTransactionOwn.setSymbol(rewardPromotionSetting.getCoin().getUnit());
               memberTransactionOwn.setType(TransactionType.PROMOTION_AWARD);
               memberTransactionOwn.setMemberId(member.getId());
               memberTransactionOwn.setRealFee("0");
               memberTransactionOwn.setDiscountFee("0");
               memberTransactionOwn.setCreateTime(new Date());
               memberTransactionService.save(memberTransactionOwn);

           }
           if(member.getInviterId()!=null) {
                Member member1 = memberDao.findOne(member.getInviterId());
                promotion(member1, member,rewardPromotionSetting);
...
//        bhbChangeService.saveBhbChange(BigDecimal.valueOf(10000),memberId,"1","新用户赠送");
    ...
    private void promotion(Member member1, Member member,RewardPromotionSetting rewardPromotionSetting) {
        ...
        if (rewardPromotionSetting != null) {
            ...
            if(memberWallet1.getToReleased()==null){
                memberWallet1.setToReleased(BigDecimal.ZERO);
            }
            //获取邀请奖励
