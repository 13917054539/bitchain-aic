...
public interface AdvertiseDao extends BaseDao<Advertise> {
    ...
    /** 
     * 功能描述: 查询上架中的CTO信息
     * @param: advertiseType 0是够买  1是出售
     * @return:
     */
    @Query(value = "SELECT * \n" +
            "FROM advertise \n" +
            "WHERE coin_id=7 AND `status`=0 AND advertise_type=:advertiseType AND  ORDER BY update_time ASC ",nativeQuery = true)
    List<Advertise> allCtoAdvertise(@Param("advertiseType")Integer advertiseType);


    /**
     * 功能描述: 查询上架中的CTO信息
     * @param: advertiseType 0是够买
     * @return:
     */
    @Query(value = "SELECT * \n" +
            "FROM advertise \n" +
            "WHERE coin_id=7 AND `status`=0 AND advertise_type=0 AND number=:number AND deal_amount=0 ORDER BY update_time ASC LIMIT 1",nativeQuery = true)
    Advertise allShellCtoAdvertise(@Param("number")BigDecimal number);
