...
public class MemberEvent {
    private Logger logger = LoggerFactory.getLogger(MemberEvent.class);
    ...
    public void onRegisterSuccess(Member member, String promotionCode) throws InterruptedException {
        ...
        handle(json.toJSONString());
        MemberWallet memberWallet = memberWalletService.findByCoinUnitAndMemberId("USDT", member.getId());
        if(memberWallet==null|| org.apache.commons.lang.StringUtils.isEmpty(memberWallet.getAddress())){
            kafkaTemplate.send("reset-member-address", "USDT", json.toJSONString());
        }
        //推广活动
        if (StringUtils.hasText(promotionCode)) {
            ...
            if (member1 != null) {
                ...
                if(member1.getGeneration()!=null){
                    member.setGeneration(member1.getGeneration()+1);
                }
                if(StringUtils.isEmpty(member1.getGenes())){
                    member.setGenes(member1.getId().toString());
                }else {
                    member.setGenes(member1.getGenes()+","+member1.getId());
                }
                //如果不需要实名认证，直接发放奖励
               if (needRealName == 0) {
                ...
                //log.info(">>>>>>>>>>注册奖励>>>>>>>>>>");
                //aefPatternService.registrationAward(member.getId(),member1.getId());
            }else{
                log.info(promotionCode+"推荐码不存在");
            }
        }else{
    ...
    public void handle(String content) {
        ...
        for(Coin coin:coins) {
            ...
            wallet.setToReleased(new BigDecimal(0));
            wallet.setConvertible(new BigDecimal(0));
            wallet.setConvertiblequota(new BigDecimal(0));
            wallet.setAddress("");
            //wallet.setCtobalance(BigDecimal.ZERO);
            wallet.setCtoquota(BigDecimal.ZERO);
