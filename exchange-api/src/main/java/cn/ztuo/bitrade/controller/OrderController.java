...
public class OrderController {
    ...
    public MessageResult addOrder(@SessionAttribute(SESSION_MEMBER) AuthMember authMember,
       ...
       /* if(DateUtil.compareTime()){
        ...
        if(symbol.equals("AIOS/USDT")){
            if(authMember.getId()==10000L||authMember.getId()==13491L||authMember.getId()==13545L){
            }else{
                return MessageResult.error("AIOS/USDT未开放");
            }
        }
        if (memberId != null) {
            authMember.setId(memberId);
        }
...
//        }
        ...
        //是否是卖，并且设置了最低卖价不为0，并且(价格小于最低卖价或者此单为市价卖单)
        if (direction == ExchangeOrderDirection.SELL
                && exchangeCoin.getMinSellPrice().compareTo(BigDecimal.ZERO) > 0
     ...
     * @return
     */
    @RequestMapping("addRobotOrder")
    public MessageResult addRobotOrder(
                                  ExchangeOrderDirection direction, String symbol, BigDecimal price,
                                  BigDecimal amount, ExchangeOrderType type, Integer isRobotOrder, Long memberId) {
       /* if(DateUtil.compareTime()){
            return MessageResult.error("交易停止中...");
...
//        }
    ...
    public MessageResult cancelOrder(@SessionAttribute(SESSION_MEMBER) AuthMember member, @PathVariable String orderId ) {
        ...
        if(order.getSymbol().equals("AIOS/USDT")){
            if(member.getId()==10000L||member.getId()==13491L||member.getId()==13545L){
            }else{
                return MessageResult.error("AIOS/USDT未开放");
            }
        }
        if (isExchangeOrderExist(order)) {
            ...
            RobotOrder robotOrder = robotOrderService.findByExchangeOrderId(exchangeOrder.getOrderId());
            if (robotOrder != null) {
                if(robotOrder.getType()==1){
                    return MessageResult.error(500,"机器人发布卖单不允许撤回");
                }
                ...
                if (new Date().compareTo(cal.getTime()) >= 0) {
                    ...
                    log.info("当前成交量"+exchangeOrder.getTradedAmount());
                    if (exchangeOrder.getTradedAmount().compareTo(BigDecimal.ZERO) == 0) {
                        ...
                        return MessageResult.error("机器人订单已有交易不允许撤回委托");
                    }
                }
            } else {
                // 发送消息至Exchange系统
    ...
    public static void main(String[] args) {
        ...
        System.out.println(new Date().compareTo(cal.getTime()) >= 0);
