...
public class CoinExchangeRate {
    @Getter
    @Setter
    private BigDecimal usdCnyRate = new BigDecimal("6.92348");
    @Getter
    @Setter
    ...
    @Getter
    @Setter
    private BigDecimal usdBtcRate = new BigDecimal("0.0000876584");
    @Getter
    @Setter
    private BigDecimal usdEthRate = new BigDecimal("0.00344");
    ...
    public BigDecimal getUsdRate(String symbol) {
        ...
        if ("USDT".equalsIgnoreCase(symbol)) {
            ...
            //BigDecimal bigDecimal = BigDecimal.ONE.divide(usdCnyRate, 4,BigDecimal.ROUND_DOWN).setScale(4, BigDecimal.ROUND_DOWN);
            BigDecimal bigDecimal = BigDecimal.ONE.divide(usdEthRate, 4,BigDecimal.ROUND_DOWN).setScale(4, BigDecimal.ROUND_DOWN);
            return bigDecimal;
            ...
            return bigDecimal;
        }else if ("Bitcoin".equalsIgnoreCase(symbol)) {
            BigDecimal bigDecimal = BigDecimal.ONE.divide(usdBtcRate, 4,BigDecimal.ROUND_DOWN).setScale(4, BigDecimal.ROUND_DOWN);
            return bigDecimal;
        }
    ...
    public void syncPrice() throws UnirestException {
        String url = "https://api.1forge.com/quotes";
        HttpResponse<JsonNode> resp = Unirest.get(url)
                .queryString("pairs", "USDCNH,USDJPY,USDHKD,SGDCNH,USDBTC,USDETH")
                .queryString("api_key", "yfl9EvU5ICKrFASuhXvr3yzgDydLfWSN")
                .asJson();
        ...
        result.forEach(json -> {
            ...
            if ("USDCNH".equals(obj.getString("symbol"))) {
                ...
                log.info(obj.toString());
            }else if("USDBTC".equals(obj.getString("symbol"))){
                setUsdBtcRate(new BigDecimal(obj.getDouble("price")).setScale(16,RoundingMode.DOWN));
                log.info(obj.toString());
            }else if("USDETH".equals(obj.getString("symbol"))){
                setUsdEthRate(new BigDecimal(obj.getDouble("price")).setScale(16,RoundingMode.DOWN));
                log.info(obj.toString());
            }
        });
    }

    public void realSyncPrice() throws UnirestException {
        String url = "https://api.1forge.com/quotes";
        HttpResponse<JsonNode> resp = Unirest.get(url)
                .queryString("pairs", "USDCNH,USDJPY,USDHKD,SGDCNH,USDBTC,USDETH")
                .queryString("api_key", "yfl9EvU5ICKrFASuhXvr3yzgDydLfWSN")
