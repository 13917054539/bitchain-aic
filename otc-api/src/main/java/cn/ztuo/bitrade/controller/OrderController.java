...
public class OrderController {
   ...
   /* private static Logger logger = LoggerFactory.getLogger(OrderController.class);*/
    ...
    public MessageResult sell(long id, long coinId, BigDecimal price, BigDecimal money,
        ...
        //扣除额度
        //如果是下架CTO则还需要补回额度
        if(otcCoin.getName().equals("CTO")){
            //获取CTO钱包信息
            MemberWallet memberWalletCto = memberWalletService.findByOtcCoinAndMemberId(otcCoin, wallet.getMemberId());
            String s = bIPBMathService.updateCtoquota(memberWalletCto.getId(), amount, 0, memberWalletCto.getCoin().getName(), TransactionType.BUYING_COINS);
            if(s.length()>0){
                return MessageResult.error("额度不足");
            }
        }
        isTrue(compare(advertise.getRemainAmount(), amount), msService.getMessage("AMOUNT_NOT_ENOUGH"));
        ...
        if (ret == 1) {
            ...
            if (result.getCode() == 0) {
                ...
                if(order.getCoin().getName().equals("CTO")){
                    //获取CTO钱包信息
                    MemberWallet memberWalletCto = memberWalletService.findByOtcCoinAndMemberId(order.getCoin(), order.getCustomerId());
                    bIPBMathService.updateCtoquota(memberWalletCto.getId(), order.getNumber(), 1, memberWalletCto.getCoin().getName(), TransactionType.CANCELLATION_OF_ORDER);
                }
            ...
            if (result.getCode() == 0) {
                ...
                if(order.getCoin().getName().equals("CTO")){
                    //获取CTO钱包信息
                    MemberWallet memberWalletCto = memberWalletService.findByOtcCoinAndMemberId(order.getCoin(), order.getMemberId());
                    bIPBMathService.updateCtoquota(memberWalletCto.getId(), add(order.getNumber(), order.getCommission()), 1, memberWalletCto.getCoin().getName(), TransactionType.CANCELLATION_OF_ORDER);
                }
                return MessageResult.success(msService.getMessage("CANCEL_SUCCESS"));
            } else {
                throw new InformationExpiredException("Information Expired");
