...
//        }
    ...
    public MessageResult detail(Long id, @SessionAttribute(SESSION_MEMBER) AuthMember shiroUser) {
       ...
       if(advertise.getCoinUnit().equals("CTO")){
           AefPatternConf aefPatternConf = aefPatternConfDao.getAefPatternConf(1);
           advertise.setMarketPrice(aefPatternConf.getCtocurrency());
       }else {
    ...
    public MessageResult putOnShelves(long id, @SessionAttribute(SESSION_MEMBER) AuthMember authMember) throws InformationExpiredException {
        ...
        if (advertise.getAdvertiseType().equals(AdvertiseType.SELL)) {
            ...
            if(otcCoin.getName().equals("CTO")){
                //获取CTO钱包信息
                MemberWallet memberWalletCto = memberWalletService.findByOtcCoinAndMemberId(otcCoin, authMember.getId());
                String result = bIPBMathService.updateCtoquota(memberWalletCto.getId(), advertise.getNumber(), 0, memberWalletCto.getCoin().getName(), TransactionType.UPPER_SHELF);
                if(result.length()>0){
    ...
    public MessageResult putOffShelves(long id, @SessionAttribute(SESSION_MEMBER) AuthMember authMember) throws InformationExpiredException {
        ...
        if (advertise.getAdvertiseType().equals(AdvertiseType.SELL)) {
            ...
            if(otcCoin.getName().equals("CTO")){
                //获取CTO钱包信息
                MemberWallet memberWalletCto = memberWalletService.findByOtcCoinAndMemberId(otcCoin, authMember.getId());
                bIPBMathService.updateCtoquota(memberWalletCto.getId(), advertise.getRemainAmount(), 1, memberWalletCto.getCoin().getName(), TransactionType.LOWER_SHELF);
