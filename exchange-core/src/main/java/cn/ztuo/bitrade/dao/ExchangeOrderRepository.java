...
public interface ExchangeOrderRepository extends JpaRepository<ExchangeOrder, String>, JpaSpecificationExecutor<ExchangeOrder>, QueryDslPredicateExecutor<ExchangeOrder> {
    ...
    @Query(value="select count(1) from exchange_order exchange where exchange.completed_time >= :startTime and exchange.completed_time <= :endTime and exchange.direction = 0 and exchange.member_id= :sellMemberId",nativeQuery = true)
    int selectBuyOrderCountByCompleteTime(@Param("startTime") long startTime,@Param("endTime")long endTime,@Param("sellMemberId")long sellMemberId);
