...
public class MemberConsumer {
    ...
    public void resetAddress(ConsumerRecord<String,String> record){
        ...
        if(coin.getEnableRpc()==BooleanEnum.IS_TRUE){
            ...
            String url;
            try{
                url= "http://" + serviceName + "/rpc/address/{account}";
                ...
                if(coin.getUnit().equals("USDT")){
                    url= "http://" + serviceName + "-ETH/rpc/address/{account}";
                    ResponseEntity<MessageResult> resultEth = restTemplate.getForEntity(url, MessageResult.class, account);
                    logger.info("remote call:service={},result={}", url, resultEth);
                    if (resultEth.getStatusCode().value() == 200) {
                        MessageResult mr = resultEth.getBody();
                        if (mr.getCode() == 0) {
                            String address =  mr.getData().toString();
                            memberWallet.setAddressEth(address);
                        }
                    }
                }
            }
...
//            }
       ...
       /* RewardActivitySetting rewardActivitySetting = rewardActivitySettingService.findByType(ActivityRewardType.REGISTER);
        if (rewardActivitySetting!=null){
            ...
            memberWallet.setToReleased(BigDecimalUtils.add(memberWallet.getBalance(),amount3));
            memberWalletService.save(memberWallet);
