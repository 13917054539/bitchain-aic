...
public class MemberTransactionController extends BaseAdminController {
    ...
    @RequiresPermissions("finance:member-transaction:page_context")
    @PostMapping("page_context")
    @AccessLog(module = AdminModule.FINANCE, operation = "分页条件获取会员交易记录")
    public MessageResult page_context(Long memberId,Integer type ,String date,Integer pageNo,Integer pageSize) {
        ...
        List<MemberTransaction> memberTransactions = memberTransactionService.getMemberTransactionByPage(paramMap);
        //获取行数
        int count=memberTransactionService.getMemberTransactionCount(paramMap);
        int totalPages=count/pageSize;
        if (count % pageSize != 0){
            totalPages ++;
        }
        //        //总收益
//        BigDecimal sum=memberTransactionService.getAmountByType(paramMap);
//        //今日收益
//        paramMap.put("toDay","toDay");
//        BigDecimal toDaySum=memberTransactionService.getAmountByType(paramMap);
        //今日生息 选中天数的生息
        paramMap.put("type",18);
        BigDecimal toDayLiveSum= memberTransactionService.getAmountByType(paramMap);
        //总生息
        paramMap.put("date",null);
        BigDecimal liveSum=memberTransactionService.getAmountByType(paramMap);
        Map<String,Object> data=new HashMap<String,Object>();
//        data.put("sum",sum);
//        data.put("toDaySum",sum);
        data.put("totalCount",count);
        data.put("totalPages",totalPages);
        data.put("liveSum",liveSum);
        data.put("toDayLiveSum",toDayLiveSum);
        data.put("memberTransactions",memberTransactions);
