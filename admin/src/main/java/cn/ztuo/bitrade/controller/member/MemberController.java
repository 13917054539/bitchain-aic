...
public class MemberController extends BaseAdminController {
    ...
    public MessageResult page(
        ...
        for (Member member:content){
            ...
            if(!org.springframework.util.StringUtils.isEmpty(member1)){
   ...
   /* @RequiresPermissions("member:audit-business")
    ...
    @RequiresPermissions("member:alter-member-password")
    @PostMapping("alter-member-password")
    @AccessLog(module = AdminModule.SYSTEM, operation = "修改会员密码")
    public MessageResult alterMemberPassword(@RequestParam("password") Integer password,
                                             @RequestParam("memberId") Long memberId) {
        Member member = memberService.findOne(memberId);
        ...
        try {
            log.info("用户："+memberId+"修改密码："+password);
            member.setPassword(Md5.md5Digest(password + member.getSalt()).toLowerCase());
        } catch (Exception e) {
            e.printStackTrace();
        }
        memberService.save(member);
