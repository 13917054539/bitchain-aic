...
public class MonitorController {
	...
	public Map<String, List<TradePlateItem>> traderPlate(String symbol) {
        ...
        if(symbol.contains(platformToken)||symbol.contains("CAL")){
			...
			Map<String,TradePlate> re = trader.getTradePlateByHuoBi(symbol);
			result.put("bid", re.get("bids").getItems());
			result.put("ask", re.get("asks").getItems());
		}
		return result;
	}
	...
	public Map<String, JSONObject> traderPlateMini(String symbol) {
        ...
        if(symbol.contains(platformToken)||symbol.contains("CAL")){
			...
			Map<String,TradePlate> re = trader.getTradePlateByHuoBi(symbol);
			result.put("bid", re.get("bids").toJSON() );
			result.put("ask", re.get("asks").toJSON());
		}
		return result;
	}
