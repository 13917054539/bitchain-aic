...
public class CoinTrader {
    ...
    public void initialize(){
        ...
        //买单队列价格降序排列:创建一个空TreeMap，按照指定的comparator排序
        buyLimitPriceQueue = new TreeMap<>(Comparator.reverseOrder());
        //卖单队列价格升序排列:创建一个空TreeMap，按照指定的comparator排序
     ...
     * 流程:1.判断订单是否是限价订单,不是则退出
     * 2.判断限价订单是买入还是卖出
     * 3.如果是买入则加入到买盘盘口信息集合中,并发送盘口变化信息
     * 4.如果是卖出则加入到卖盘盘口信息集合中,并发送盘口变化信息
     * 5.将统一价位的订单添加到mergeOrder
     * @param exchangeOrder
     */
    public void addLimitPriceOrder(ExchangeOrder exchangeOrder){
        //ExchangeOrderType.LIMIT_PRICE:限价
        ...
        synchronized (list) {//list就是queue，把limit_order加入queue
            //getPrice():挂单价格
            //合并订单
    ...
    public void trade(ExchangeOrder exchangeOrder){
        ...
        if(!symbol.equalsIgnoreCase(exchangeOrder.getSymbol())){
            //exchangeOrder.getCoinSymbol():币单位  exchangeOrder.getBaseSymbol():结算单位
            logger.info("unsupported symbol,coin={},base={}", exchangeOrder.getCoinSymbol(), exchangeOrder.getBaseSymbol());
            return ;
        }
        //getAmount():买入或卖出量，对于市价买入单表  getTradedAmount():成交量
        ...
        //判断价格是市价还是限价
        if(exchangeOrder.getType() == ExchangeOrderType.MARKET_PRICE){
            logger.info(">>>>>市价单>>>与限价单交易");


            //与限价单交易,市价委托单与限价对手单列表交易
            ...
            logger.info(">>>>>限价单>>>与限价单交易");


            //先与限价单交易,限价委托单与限价队列匹配
    ...
    public void matchLimitPriceWithLPList(TreeMap<BigDecimal,MergeOrder> lpList, ExchangeOrder focusedOrder,boolean canEnterList){
        ...
        //如果还没有交易完，并且可以加入列表时，订单压入列表中，canEnterList=false
    ...
    private BigDecimal adjustMarketOrderTurnover(ExchangeOrder order, BigDecimal dealPrice){
        ...
        if(order.getDirection() == ExchangeOrderDirection.BUY && order.getType() == ExchangeOrderType.MARKET_PRICE){
            ...
            BigDecimal leftTurnover = order.getAmount().subtract(order.getTurnover());
            //除以  //coinScale交易币种的精度
            //保留小数点coinScale位数,第三个参数为取值精度,例如四舍五入
    ...
    public Map<String,TradePlate> getTradePlateByHuoBi(String symbol){
        Map<String,TradePlate> map = Maps.newHashMap();
        String symbol2 =  symbol.replace("/","").toLowerCase();
        String url = "https://api.huobi.pro/market/depth?symbol="+symbol2+"&type=step0";
        ...
        TradePlate bidsTradePlate =  new TradePlate();
        bidsTradePlate.setDirection(ExchangeOrderDirection.BUY);
        bidsTradePlate.setSymbol(symbol);
        TradePlate asksTradePlate =  new TradePlate();
        asksTradePlate.setDirection(ExchangeOrderDirection.SELL);
        asksTradePlate.setSymbol(symbol);

        JSONObject jsonReult = JSON.parseObject(resp.getBody().toString());
        JSONObject jsonObject = jsonReult.getJSONObject("tick");
        LinkedList<TradePlateItem> bidsItems = new LinkedList<>();
        LinkedList<TradePlateItem> asksItems = new LinkedList<>();
        if(jsonObject!=null){
            ...
            for(int i=0;i<20;i++){
                ...
                asksItems.add(tradePlateItem);
            }
        }
        bidsTradePlate.setItems(bidsItems);
        asksTradePlate.setItems(asksItems);
        map.put("bids",bidsTradePlate);
        map.put("asks",asksTradePlate);
        return map;
    }
