...
import cn.ztuo.bitrade.vo.AppRevisionVO;
...
public class AideController extends BaseController{
    ...
    public MessageResult sysHelp(@PathVariable(value = "id") Platform platform) {
        ...
        if(revision != null){
            AppRevisionVO appRevisionVO=new AppRevisionVO();
            appRevisionVO.setDownloadUrl(revision.getDownloadUrl());
            appRevisionVO.setId(revision.getId());
            appRevisionVO.setPlatform(revision.getPlatform());
            appRevisionVO.setPublishTime(revision.getPublishTime());
            appRevisionVO.setRemark(revision.getRemark());
            appRevisionVO.setVersion(revision.getVersion());
            List<String> strings=new ArrayList<>();
            strings.add("1.优化了部分UI设计");
            strings.add("2.修改了已知bug");
            appRevisionVO.setStringList(strings);
            MessageResult result = MessageResult.success();
            result.setData(appRevisionVO);
            return result;
        }
        else{
