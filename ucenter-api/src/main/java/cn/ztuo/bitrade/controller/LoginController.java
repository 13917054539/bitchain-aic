...
public class LoginController extends BaseController {
    ...
    private LoginInfo getLoginInfo(String username, String password, String ip, HttpServletRequest request) throws Exception {
        ...
        for(Coin coin:coins) {
            if(memberWalletService.findByCoinAndMemberId(coin,loginInfo.getId())!=null){
                continue;
            }
            log.info("memberId:{},unit:{}", loginInfo.getId(), coin.getUnit());
            MemberWallet wallet = new MemberWallet();
            wallet.setCoin(coin);
            wallet.setMemberId(loginInfo.getId());
            wallet.setBalance(BigDecimal.ZERO);
            wallet.setFrozenBalance(BigDecimal.ZERO);
            wallet.setConvertible(BigDecimal.ZERO);
            wallet.setToReleased(BigDecimal.ZERO);
            wallet.setConvertiblequota(BigDecimal.ZERO);
